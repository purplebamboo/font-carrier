...
var _getViewBox = function(svgNode){
  ...
  if (viewbox && viewbox.length === 4) {
    return _.map(viewbox,function(v){return parseFloat(v)})
  }
...
// #=> [512, 1024]
var normalizeViewport = function(viewport,targetHeight){
...
function _normalizeXY(view_box,targetViewbox){
  var x,y,targetX,targetY

  x = viewbox[0]
  y = viewbox[1]
  targetX = target_view_box[0]
  targetY = target_view_box[1]
  //可能会有x,y偏移的情况，所以这边需要做出相应的转换
  if (x != target_x || y != target_y) {
    return [['translate', [target_x - x, target_y - y]]]
  }
  return []
...
function _normalizeWidthHeight(viewbox,targetViewbox){
  ...
  var transforms = [],newWidth,newHeight
...
var _normalizeViewBox = function(viewbox,targetViewbox){
  ...
  var transforms = []
  transforms.concat(_normalizeXY(view_box,targetViewbox))
  transforms.concat(_normalizeWidthHeight(view_box,targetViewbox))
  return transforms
}
...
exports.generateAmendTrans = function(svgNode,targetHeight){
  ...
  var targetViewbox = normalizeViewport(viewport,targetHeight)

  return _normalizeViewBox(viewbox,targetViewbox)
